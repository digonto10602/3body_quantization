...
def QC3_bissection(pointA, pointB, K3iso1, K3iso2, nPx, nPy, nPz, nmax, tol, spline_size):
    A = pointA 
    B = pointB 
...
def K3iso_fitting_function(x0, nPx, nPy, nPz, nmax, tol, spline_size, corr_mat_inv):
    ...
    for i in range(E_size):
        ...
        QC_spectrum = QC3_bissection(Energy_A_CM, Energy_B_CM, K3iso_1, K3iso_2, nPx, nPy, nPz, nmax, tol, spline_size)
    ...
    for i in range(E_size):
        for j in range(E_size):
            ...
            jterm = (np_Elatt_CM_selected[j] - np_E_QC_CM[j])
...
def test():
    ...
    res = scipy.optimize.minimize(K3iso_fitting_function,x0=x0,args=(nPx, nPy, nPz, nmax, tol, spline_size, corr_mat_inv))
