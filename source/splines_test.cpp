        ...
        )
{
    ...
    for(int j=size-1; j!=-1; --j)
    {
        for(int i=size-1; i!=-1; --i)
        {
            ...
            {
                Cji_vec[j][i] = qj_vec[j]*Cji[j+1][i] + Aji[j][i];
            } 
        }
    }
}
...
void Sij(   std::vector<std::vector<double> > &Sij_vec, 
            double x, 
            std::vector<double> xj_vec, 
            std::vector<std::vector<double> > Cji   )
{
    int size = xj_vec.size(); 
    ...
    for(int i=0; i<size; ++i)
    {
        ...
        {   
            ...
            double term2 = (hjplus1/6.0)*(2.0*Cji[j][i] + Cji[j+1][i]) ;
            double term3 = 0.5*Cji[j][i] + xminusxj*(1.0/(6.0*hjplus1))*(Cji[j+1][i] - Cji[j][i]) ;
