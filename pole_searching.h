                                    ...
                                    double &pole   )
{
    ...
    for(int i=0; i<max_iteration; ++i)
    {
        ...
        config_maker_1(p1_config, En1 , total_P, mi, mj, mk, L, epsilon_h, config_tolerance );

        std::vector< std::vector<comp> > k1_config = p1_config;

        double En2 = point2; 
        std::vector< std::vector<comp> > p2_config(3,std::vector<comp> ());
        config_maker_1(p2_config, En2 + ii*comp_en_additive, total_P, mi, mj, mk, L, epsilon_h, config_tolerance );

        std::vector< std::vector<comp> > k2_config = p2_config;

        double f1 = det_F3_ND_2plus1_mat( point1 + ii*comp_en_additive, p1_config, k1_config, total_P, eta_i_1, eta_i_2, scattering_length_1, scattering_length_2, m_pi, m_K, alpha, epsilon_h, L, max_shell_num); 
        double f2 = det_F3_ND_2plus1_mat( point2 + ii*comp_en_additive, p2_config, k2_config, total_P, eta_i_1, eta_i_2, scattering_length_1, scattering_length_2, m_pi, m_K, alpha, epsilon_h, L, max_shell_num); 

        point0 = point1 - f1*(point1 - point2)/(f1 - f2);

        double En0 = point0; 
        std::vector< std::vector<comp> > p0_config(3,std::vector<comp> ());
        config_maker_1(p0_config, En0 + ii*comp_en_additive, total_P, mi, mj, mk, L, epsilon_h, config_tolerance );

        std::vector< std::vector<comp> > k0_config = p0_config;


        double f0 = det_F3_ND_2plus1_mat( point0 + ii*comp_en_additive, p0_config, k0_config, total_P, eta_i_1, eta_i_2, scattering_length_1, scattering_length_2, m_pi, m_K, alpha, epsilon_h, L, max_shell_num); 
...
void test_F3inv_pole_searching()
{
    ...
    for(int i=0; i<Lpoints+1; ++i)
    {
        ...
        det_F3mat_poles_secant_method(init_guess1,init_guess2, total_P, eta_1, eta_2, scattering_length_1_piK,scattering_length_2_KK, atmK, atmpi, alpha, epsilon_h, L, max_shell_num, pole);
